# Projects

These case studies are the strongest current evidence of how I investigate problems, design systems, document decisions, and consider user impact.

## Public-Benefits Application QA Investigation

**Real-world QA case study**

I documented three material problems encountered during an ordinary public-benefits application workflow: ambiguous evidence mapping, incorrect OCR or entity extraction, and validation feedback that did not match the visible data.

**Skills demonstrated:** Exploratory testing · Black-box testing · Defect documentation · Risk assessment · Acceptance criteria

[Read the public-benefits application case study](../qa-report/report.md)

## Tana Organization

**Systems design case study**

I designed a private personal knowledge system around how I naturally capture, process, connect, retrieve, and act on information.

**Skills demonstrated:** Systems thinking · Information architecture · Workflow design · Automation judgment · Documentation

[Read the Tana Organization case study](tana-organization.md)

## ShiftRight

**Web application QA case study**

I created and tested a worker-focused shift-tracking application using build verification, privacy-aware exploratory testing, a controlled synthetic guest journey, and calculation checks.

**Skills demonstrated:** Web application testing · Privacy testing · Expected-versus-actual analysis · CI verification · Synthetic test data

[Read the ShiftRight web application QA case study](shiftright.md)

## Work still being developed

Additional QA projects remain private until their tests have been executed, results verified, and evidence reviewed for safe publication. Planned or incomplete work is not presented as completed evidence.

---

[Return to the portfolio home](../README.md) · [View the Skills Map](../skill-evidence/skills.md)
